...
function setup(metadata: Record<string, unknown>, member = true) {
  ...
  mock.server.mockResolvedValue({
    auth: { getUser },
    from: () => ({ select: () => ({ eq: () => ({ maybeSingle: async () => ({ data: member ? { user_id: 'validated-id', username: 'test', full_name: 'Prueba' } : null }) }) }) }),
  })
  return getUser
}
